...
int (* const call_vec[NR_PM_CALLS])(void) = {
	...
	CALL(PM_PRINTMSG)	= do_printmsg,		/* printmsg(2) */
	CALL(PM_PSEM_INIT) = do_psem_init,
	CALL(PM_PSEM_WAIT) = do_psem_wait,
	CALL(PM_PSEM_SIGNAL) = do_psem_signal,
	CALL(PM_PSEM_DESTROY) = do_psem_destroy,
	CALL(PM_PSEM_NO) = do_psem_no ,
	CALL(PM_PSEM_BLOCK) = do_psem_block,
	CALL(PM_PSEM_WAKEUP) = do_psem_wakeup
};
